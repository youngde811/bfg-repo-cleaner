...
import com.madgag.git.gitclean.BuildInfo
...
object CLIConfig {
  val parser = new OptionParser[CLIConfig]("gitclean") {
    ...
    fileMatcher("delete-files").abbr("D").text("delete files with the specified names (eg '*.class', '*.{txt,log}' - matches "
      + "on file name, not path within repo)").action {
    ...
    opt[File]("replace-text").abbr("rt").valueName("<expressions-file>").text("filter content of files, replacing matched text. Match expressions should be "
      + "listed in the file, one expression per line - " +
    ...
    fileMatcher("filter-content-including").abbr("fi").text("do file-content filtering on files that match the "
      + "specified expression (eg '*.{txt,properties}')").action {
      (v, c) => c.copy(filenameFilters = c.filenameFilters :+ Include(v))
    }

    fileMatcher("filter-content-excluding").abbr("fe").text("don't do file-content filtering on files that match the specified "
      + "expression (eg '*.{xml,pdf}')").action {
      (v, c) => c.copy(filenameFilters = c.filenameFilters :+ Exclude(v))
    }

    opt[String]("filter-content-size-threshold").abbr("fs").valueName("<size>").text("only do file-content filtering on files "
      + "smaller than <size> (default is %1$d bytes)".format(CLIConfig().filterSizeThreshold)).action {
      (v, c) => c.copy(filterSizeThreshold = ByteSize.parse(v))
    }

    opt[String]('p', "protect-blobs-from").valueName("<refs>").text("protect blobs that appear in the most recent versions of "
      + "the specified refs (default is 'HEAD')").action {
    ...
    opt[Unit]("strict-object-checking").text("perform additional checks on integrity of consumed and created objects").hidden().action {
    ...
    opt[String]("massive-non-file-objects-sized-up-to").valueName("<size>").text("increase memory usage to handle over-size Commits, Tags, "
      + "and Trees that are up to X in size (eg '10M')").action {
      (v, c) => c.copy(massiveNonFileObjects = Some(ByteSize.parse(v)))
    }

    opt[String]("fix-filename-duplicates-preferring").valueName("<filemode>").text("Fix corrupt trees which contain multiple entries "
        + "with the same filename, favouring the 'tree' or 'blob'").hidden().action {
      (v, c) =>
